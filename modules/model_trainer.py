...
def train(
    ...
    @tf.function
    def train_step(model, image_sequences, path_sequences, labels, feature, dInt, dLon, dLat):
    ...
    # path-prediction
    @tf.function
    def train_step_path(model, image_sequences, path_sequences, labels, feature, dInt, dLon, dLat):
        with tf.GradientTape() as tape:  # Creates a context manager GradientTape used for computing gradients.
            model_output = model(image_sequences, feature, training=True)  # iterate training.
    ...
    #     output = model(image_sequences, feature, training=False)
